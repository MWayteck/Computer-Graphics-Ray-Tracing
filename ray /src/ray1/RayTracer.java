...
public class RayTracer {
	...
	public static void shadeRay(Colorf outColor, Scene scene, Ray ray) 
	{
		...
		if(!scene.getFirstIntersection(intersect, ray))
		{
			outColor.set(scene.getBackColor());
			return;
		}
		
		Shader shader = intersect.surface.getShader();
		shader.shade(outColor, scene, ray, intersect);
	}

	public static void shadeRay(Colorf outColor, Scene scene, Ray rRay, int depth) {
		// TODO Auto-generated method stub
		
		IntersectionRecord intersect = new IntersectionRecord();
		
		if(!scene.getFirstIntersection(intersect, rRay))
		{
			outColor.set(scene.getBackColor());
			return;
		}
		...
		Shader shader = intersect.surface.getShader();
		shader.shade(outColor, scene, rRay, intersect);
	}
}