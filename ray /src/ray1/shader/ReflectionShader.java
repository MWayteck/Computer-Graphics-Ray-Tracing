...
public abstract class ReflectionShader extends Shader {
	...
	public void shade(Colorf outIntensity, Scene scene, Ray ray, IntersectionRecord record, int depth) {
		...
		for(Light light:scene.getLights())
		{
			...
			incoming.set(light.position).sub(record.location);
			...
			outIntensity.add(BRDFVal.clone().mul(light.intensity)
					.mul((float)(Math.max(surfaceNormal.dot(incoming),0)/r2)));
		...
		Vector3d d = outgoing.clone().negate();
		Vector3d r = d.subMultiple(2*d.dot(surfaceNormal),surfaceNormal);
		Ray rRay = new Ray(record.location, r);
		rRay.makeOffsetRay();
		double cosTheta = surfaceNormal.dot(outgoing);
		Vector3 reflectance = getMirrorCoefficient()
				.addMultiple((float)Math.pow(1-cosTheta,5), new Colorf(1,1,1).sub(getMirrorCoefficient()));
		Colorf outColor = new Colorf();
		RayTracer.shadeRay(outColor,scene,rRay,depth+1);
